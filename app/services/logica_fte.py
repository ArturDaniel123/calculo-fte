...
def calcular_fte(salario_hora, tempo_tarefa, repeticao, tempo_bot,
    ...
    if salario_hora <= 0:
        raise FTEValidationError("O salário não pode ser igual ou menor que zero.")
    salario_minuto = salario_hora/60
    
    if tempo_tarefa <= 0:
        raise FTEValidationError("O tempo de realização da tarefa não pode ser igual ou menor que zero.")   
    custo_por_tarefa_humana = tempo_tarefa * salario_minuto
    
    if repeticao <= 0:
        raise FTEValidationError("O número de repetições não pode ser nulo.") 
    ...
    if custo_mensal_bot < 0:
        raise FTEValidationError("Custo mensal bot inválido.")
    econ_liq_mes = custo_mensal_humano - custo_mensal_bot # Economia liquída do mês

    if tempo_bot <= 0:
        raise FTEValidationError("O tempo de execução de tarefa não pode ser nulo.")
